...
template <typename TYPE_X, typename TYPE_Y>
__aicore__ inline void KernelFloor<TYPE_X, TYPE_Y>::Init(
    GM_ADDR x, GM_ADDR y, uint64_t smallCoreDataNum, uint64_t bigCoreDataNum, uint64_t finalBigTileNum,
    uint64_t finalSmallTileNum, uint64_t tileDataNum, uint64_t smallTailDataNum, uint64_t bigTailDataNum,
...
template <typename TYPE_X, typename TYPE_Y>
__aicore__ inline void KernelFloor<TYPE_X, TYPE_Y>::CopyIn(int32_t progress)
{
...
template <typename TYPE_X, typename TYPE_Y>
__aicore__ inline void KernelFloor<TYPE_X, TYPE_Y>::CopyOut(int32_t progress)
{
...
template <typename TYPE_X, typename TYPE_Y>
__aicore__ inline void KernelFloor<TYPE_X, TYPE_Y>::Compute(int32_t progress)
{
    ...
    if constexpr (std::is_same_v<TYPE_X, bfloat16_t>) {
        ...
        AscendC::Floor(p1, p1, this->processDataNum);
        AscendC::Cast(yLocal, p1, AscendC::RoundMode::CAST_FLOOR, this->processDataNum);
    } else {
...
template <typename TYPE_X, typename TYPE_Y>
__aicore__ inline void KernelFloor<TYPE_X, TYPE_Y>::Process()
{
