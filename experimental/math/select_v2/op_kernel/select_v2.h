    ...
    uint64_t tailBlockNum)
{
    ...
    uint32_t tileDataNumAlign256 = (this->tileDataNum + 255) / 256 * 256;
...
__aicore__ inline void KernelSelectV2<TYPE_X, TYPE_Y>::Compute(int32_t progress)
{
    ...
    AscendC::CompareScalar(condLocal.ReinterpretCast<uint8_t>(), cond_fp16Local, (half)(1.0), AscendC::CMPMODE::EQ, compareCnt);
    ...
    if constexpr ((std::is_same_v<TYPE_Y, float>) || (std::is_same_v<TYPE_Y, uint32_t>) || (std::is_same_v<TYPE_Y, int32_t>)) {
        ...
        AscendC::Select(yLocal.ReinterpretCast<float>(), condLocal.ReinterpretCast<uint8_t>(), selfLocal.ReinterpretCast<float>(), otherLocal.ReinterpretCast<float>(), 
    ...
    if constexpr ((std::is_same_v<TYPE_Y, half>) || (std::is_same_v<TYPE_Y, bfloat16_t>) 
        ...
        AscendC::Select(yLocal.ReinterpretCast<half>(), condLocal.ReinterpretCast<uint8_t>(), selfLocal.ReinterpretCast<half>(), otherLocal.ReinterpretCast<half>(), 
                        AscendC::SELMODE::VSEL_TENSOR_TENSOR_MODE, this->processDataNum); 
    ...
    if constexpr ((std::is_same_v<TYPE_Y, uint8_t>) || (std::is_same_v<TYPE_Y, int8_t>) || (std::is_same_v<TYPE_Y, bool>)) {
        ...
        AscendC::Select(p1, condLocal.ReinterpretCast<uint8_t>(), p1, p2, AscendC::SELMODE::VSEL_TENSOR_TENSOR_MODE, this->processDataNum);    
