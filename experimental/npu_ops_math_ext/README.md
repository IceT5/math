...
安装完成后，您可以像使用普通PyTorch操作一样使用NPU算子，以isfinite算子为例，您可以在`npu_ops_math_ext\csrc\is_finite\test`目录下找到并执行这个脚本:

```python
import torch
import torch_npu
import npu_ops_math_ext
...
for data_type in supported_dtypes:
    ...
    x_npu = x.npu()
    # 调用自定义接口
    npu_result = torch.ops.npu_ops_math_ext.isfinite(x_npu).cpu()
    print(f"[OK] torch.ops.npu_ops_math_ext.isfinite<{data_type}> successfully!")
...
1. 编写算子调用文件
    ...
    在 `npu_ops_math_ext/csrc/` 目录下添加新的算子目录 `mykernel`，在 `mykernel` 目录下添加新的算子调用文件 `mykernel_torch.cpp`
    ```c++
...
3. 在 `npu_ops_math_ext/csrc/npu_ops_def.cpp`中添加TORCH_LIBRARY_IMPL定义
...
4. (可选)在 `npu_ops_math_ext/ops.py`中封装自定义接口
    ...
    def mykernel(x: Tensor) -> Tensor:
        return torch.ops.npu_ops_math_ext.mykernel.default(x)
    ```
...
6. 编写测试脚本并测试新算子
    ```python
    torch.ops.npu_ops_math_ext.mykernel(x)
    ```
