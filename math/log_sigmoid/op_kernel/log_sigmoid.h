...
#include "log_sigmoid_tiling_data.h"
#include "log_sigmoid_tiling_key.h"

namespace NsLogSigmoid {
...
class LogSigmoid {
public:
    __aicore__ inline LogSigmoid(){};

    __aicore__ inline uint32_t AlignUp(uint32_t a, uint32_t b); 
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR z, const LogSigmoidTilingData &tilingData);
...
template <typename T>
__aicore__ inline uint32_t LogSigmoid<T>::AlignUp(uint32_t a, uint32_t b) 
{
    if (b == 0)
        return a;
...
template <typename T>
__aicore__ inline void LogSigmoid<T>::Init(GM_ADDR x, GM_ADDR z, const LogSigmoidTilingData &tilingData)
{
...
template <typename T>
__aicore__ inline void LogSigmoid<T>::CopyIn(int32_t progress)
{
...
template <typename T>
__aicore__ inline void LogSigmoid<T>::CopyOut(int32_t progress)
{
...
template <typename T>
__aicore__ inline void LogSigmoid<T>::Compute(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.DeQue<T>();
    AscendC::LocalTensor<T> zLocal = outputQueueZ.AllocTensor<T>();
    AscendC::LocalTensor<T> tmpLocal = inputQueueX.AllocTensor<T>();

    // -x
    AscendC::Muls(xLocal, xLocal, (T)(-1), this->processDataNum);

    // exp(-x)
    AscendC::Exp(xLocal, xLocal, this->processDataNum);

    // 1 + exp(-x)
    AscendC::Adds(xLocal, xLocal, (T)1, this->processDataNum);

    // ln(1 + exp(-x))
    AscendC::Ln(xLocal, xLocal, this->processDataNum);

    // y = -tmp
    AscendC::Muls(zLocal, xLocal, (T)(-1), this->processDataNum);
...
template <typename T>
__aicore__ inline void LogSigmoid<T>::Process()
{
