...
#include "math/log_sigmoid/op_kernel/log_sigmoid_tiling_data.h"
#include "math/log_sigmoid/op_kernel/log_sigmoid_tiling_key.h"
...
// inline static ge::graphStatus SetTilingDataForLogSigmoid(gert::TilingContext* context, LogSigmoidTilingData& tilingData)
// {
                                       ...
                                       ge::DataType& dataType, uint32_t& typeSize)
{
    ...
    OP_CHECK_IF(dimNum == 0, OP_LOGE(context, "LogSigmoid: input shape dim is 0"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(totalElements == 0, OP_LOGE(context, "LogSigmoid: total elements is 0"), return ge::GRAPH_FAILED);
    ...
    switch (dataType) {
        ...
        case ge::DT_FLOAT16:
            ...
            break;
        default:
            OP_LOGE(context, "LogSigmoid: unsupported data type");
            return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus CalculateCoreDistribution(int64_t totalElements, int64_t coreNum,
                                               LogSigmoidTilingData& tilingData)
{
    // 大核小核分配
...
static ge::graphStatus CalculateIntraCorePartition(int64_t coreNum, 
                                                 const LogSigmoidTilingData& tilingData,
...
// 计算每个核的循环参数
static void CalculateCoreLoopParams(int64_t coreNum, int32_t tileElementNum, LogSigmoidTilingData& tilingData)
{
    for (int32_t coreId = 0; coreId < coreNum; coreId++) {
        if (tilingData.core_element_count[coreId] > 0) {
...
static ge::graphStatus GetWorkspaceSize(gert::TilingContext* context)
{
    ...
    currentWorkspace[0] = 0; // LogSigmoid操作通常不需要workspace
    return ge::GRAPH_SUCCESS;
}

// Tiling分发入口
static ge::graphStatus LogSigmoidTilingFunc(gert::TilingContext* context)
{
    // 1. 获取平台运行信息
    uint64_t ubSize;
    int64_t coreNum;
    ...
    // 4. 初始化tiling数据
    LogSigmoidTilingData* tiling = context->GetTilingData<LogSigmoidTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(LogSigmoidTilingData), 0, sizeof(LogSigmoidTilingData)) != EOK,
    ...
    // OP_CHECK_IF(
    //     SetTilingDataForLogSigmoid(context, tiling) != ge::GRAPH_SUCCESS,
    //     OP_LOGE(context, "PowsSetTilingData set tiling data fail."), return ge::GRAPH_FAILED);    

    // 打印调试信息
    OP_LOGI(context, "LogSigmoid Tiling: totalElements=%ld, coreNum=%ld, tileElementNum=%d", 
...
static ge::graphStatus TilingParseForLogSigmoid(gert::TilingParseContext* context)
{
    OP_LOGD(context, "TilingParseForLogSigmoid enter.");

    auto compileInfo = context->GetCompiledInfo<LogSigmoidCompileInfo>();
    ...
    OP_CHECK_IF(
        (compileInfo->totalCoreNum <= 0), OP_LOGE(context, "TilingParseForLogSigmoid fail to get core num."),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        (compileInfo->ubSizePlatForm <= 0), OP_LOGE(context, "TilingParseForLogSigmoid fail to get ub size."),
        return ge::GRAPH_FAILED);
    OP_LOGD(context, "TilingParseForLogSigmoid exit.");
    return ge::GRAPH_SUCCESS;
}

// Tiling注册入口
IMPL_OP_OPTILING(LogSigmoid).Tiling(LogSigmoidTilingFunc).TilingParse<LogSigmoidCompileInfo>(TilingParseForLogSigmoid);
} // namespace optiling