...
/*!
 * \file log_sigmoid_graph_infer.cpp
 * \brief log_sigmoid operater graph infer resource
 */
...
static ge::graphStatus InferDataTypeLogSigmoid(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeLogSigmoid");

    // 设置输出的dtype
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeLogSigmoid");
    return GRAPH_SUCCESS;
}

IMPL_OP(LogSigmoid).InferDataType(InferDataTypeLogSigmoid);
