...
#include "math/relu/op_kernel/relu_tiling_data.h"
#include "math/relu/op_kernel/relu_tiling_key.h"
...
// inline static ge::graphStatus SetTilingDataForRelu(gert::TilingContext* context, ReluTilingData& tilingData)
// {
                                       ...
                                       ge::DataType& dataType, uint32_t& typeSize)
{
    ...
    OP_CHECK_IF(dimNum == 0, OP_LOGE(context, "Relu: input shape dim is 0"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(totalElements == 0, OP_LOGE(context, "Relu: total elements is 0"), return ge::GRAPH_FAILED);

    // dtype校验
    const std::set<ge::DataType> supportedDtype = {ge::DT_FLOAT, ge::DT_FLOAT16, ge::DT_INT32, ge::DT_INT16, ge::DT_INT8};
    ...
    switch (dataType) {
        ...
        case ge::DT_INT8:
            ...
            break;
        default:
            OP_LOGE(context, "Relu: unsupported data type");
            return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
static uint32_t EstimateUBUsage(int32_t elem_num, uint32_t typeSize)
{
    ...
    // // 工作区Buffer
    // uint32_t workBytes = AlignUp(elem_num * typeSize, BLOCK_SIZE);
    // 双缓冲总需求
...
// 计算每个核的循环参数
static void CalculateCoreLoopParams(int64_t coreNum, int32_t tileElementNum, ReluTilingData& tilingData)
{
    for (int32_t coreId = 0; coreId < coreNum; coreId++) {
        if (tilingData.core_element_count[coreId] > 0) {
...
static ge::graphStatus GetWorkspaceSize(gert::TilingContext* context)
{
    ...
    currentWorkspace[0] = 0; // Relu操作通常不需要workspace
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus ReluTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        memset_s(tiling, sizeof(ReluTilingData), 0, sizeof(ReluTilingData)) != EOK,
    ...
    // OP_CHECK_IF(
    //     SetTilingDataForRelu(context, tiling) != ge::GRAPH_SUCCESS,
    //     OP_LOGE(context, "PowsSetTilingData set tiling data fail."), return ge::GRAPH_FAILED);    

    // 打印调试信息
    OP_LOGI(context, "Relu Tiling: totalElements=%ld, coreNum=%ld, tileElementNum=%d", 
...
{
    OP_LOGD(context, "TilingParseForRelu enter.");

    auto compileInfo = context->GetCompiledInfo<ReluCompileInfo>();
    ...
    OP_CHECK_IF(
        (compileInfo->totalCoreNum <= 0), OP_LOGE(context, "TilingParseForRelu fail to get core num."),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        (compileInfo->ubSizePlatForm <= 0), OP_LOGE(context, "TilingParseForRelu fail to get ub size."),
        return ge::GRAPH_FAILED);
    OP_LOGD(context, "TilingParseForRelu exit.");
    return ge::GRAPH_SUCCESS;
}
