 ...
 * \brief hard_swish operater graph infer resource
 */
...
static ge::graphStatus InferDataTypeHardSwish(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeHardSwish");

    // 设置输出的dtype
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeHardSwish");
    return GRAPH_SUCCESS;
}

IMPL_OP(HardSwish).InferDataType(InferDataTypeHardSwish);
