                                       ...
                                       ge::DataType& dataType, uint32_t& typeSize)
{
    ...
    OP_CHECK_IF(dimNum == 0, OP_LOGE(context, "Cosh: input shape dim is 0"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(totalElements == 0, OP_LOGE(context, "Cosh: total elements is 0"), return ge::GRAPH_FAILED);
    ...
    switch (dataType) {
        ...
        case ge::DT_FLOAT16:
            ...
            break;
        default:
            OP_LOGE(context, "Cosh: unsupported data type");
            return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus CalculateIntraCorePartition(int64_t coreNum, 
                                                 const CoshTilingData& tilingData,
...
// 计算每个核的循环参数
static void CalculateCoreLoopParams(int64_t coreNum, int32_t tileElementNum, CoshTilingData& tilingData)
{
    for (int32_t coreId = 0; coreId < coreNum; coreId++) {
        if (tilingData.core_element_count[coreId] > 0) {
...
static ge::graphStatus GetWorkspaceSize(gert::TilingContext* context)
{
    ...
    currentWorkspace[0] = 0; // Cosh操作通常不需要workspace
    return ge::GRAPH_SUCCESS;
}
