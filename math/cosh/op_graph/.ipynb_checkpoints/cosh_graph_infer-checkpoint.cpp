...
/*!
 * \file cosh_graph_infer.cpp
 * \brief cosh operater graph infer resource
 */
...
static ge::graphStatus InferDataTypeCosh(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeCosh");

    // 设置输出的dtype
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeCosh");
    return GRAPH_SUCCESS;
}

IMPL_OP(Cosh).InferDataType(InferDataTypeCosh);
