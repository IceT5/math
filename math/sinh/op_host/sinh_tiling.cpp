...
#include "math/sinh/op_kernel/sinh_tiling_data.h"
#include "math/sinh/op_kernel/sinh_tiling_key.h"
...
ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    // shape校验
    OP_CHECK_IF(
        inputShapeX.GetDimNum() != DIMS_LIMIT  || outShapeZ.GetDimNum() != DIMS_LIMIT,
        OP_LOGE(
            context, "Sinh: inputx,outputz shape dim = %zu, %zu, should be equal 4",
...
static ge::graphStatus SinhTilingFunc(gert::TilingContext* context)
{
    ...
    if (totalIdx <= 0) {
        ...
        memset_s(tiling, sizeof(SinhTilingData), 0, sizeof(SinhTilingData));
