...
#include "math/mul_v2/op_kernel/mul_v2_tiling_data.h"
#include "math/mul_v2/op_kernel/mul_v2_tiling_key.h"
...
ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            context, "MulV2: inputx,inputy,outputz shape dim = %zu, %zu, %zu, should be equal 4",
...
static ge::graphStatus MulV2TilingFunc(gert::TilingContext* context)
{
    ...
    MulV2TilingData* tiling = context->GetTilingData<MulV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(MulV2TilingData), 0, sizeof(MulV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForMulV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(MulV2).Tiling(MulV2TilingFunc).TilingParse<MulV2CompileInfo>(TilingParseForMulV2);
} // namespace optiling