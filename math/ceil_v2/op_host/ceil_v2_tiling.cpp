...
#include "math/ceil_v2/op_kernel/ceil_v2_tiling_data.h"
#include "math/ceil_v2/op_kernel/ceil_v2_tiling_key.h"
                                       ...
                                       ge::DataType& dataType, uint32_t& typeSize)
{
    ...
    OP_CHECK_IF(dimNum == 0, OP_LOGE(context, "CeilV2: input shape dim is 0"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(totalElements == 0, OP_LOGE(context, "CeilV2: total elements is 0"), return ge::GRAPH_FAILED);
    ...
    switch (dataType) {
        ...
        case ge::DT_FLOAT16:
            ...
            break;
        default:
            OP_LOGE(context, "CeilV2: unsupported data type");
            return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus CalculateIntraCorePartition(int64_t coreNum, 
                                                 const CeilV2TilingData& tilingData,
...
// 计算每个核的循环参数
static void CalculateCoreLoopParams(int64_t coreNum, int32_t tileElementNum, CeilV2TilingData& tilingData)
{
    for (int32_t coreId = 0; coreId < coreNum; coreId++) {
        if (tilingData.core_element_count[coreId] > 0) {
...
static ge::graphStatus GetWorkspaceSize(gert::TilingContext* context)
{
    ...
    currentWorkspace[0] = 0; // CeilV2操作通常不需要workspace
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus CeilV2TilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        memset_s(tiling, sizeof(CeilV2TilingData), 0, sizeof(CeilV2TilingData)) != EOK,
    ...
    // 打印调试信息
    OP_LOGI(context, "CeilV2 Tiling: totalElements=%ld, coreNum=%ld, tileElementNum=%d", 
