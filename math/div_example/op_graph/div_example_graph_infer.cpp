...
/*!
 * \file div_example_graph_infer.cpp
 * \brief div_example operater graph infer resource
 */
...
static ge::graphStatus InferDataTypeDivExample(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeDivExample");

    // 设置输出的dtype
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeDivExample");
    return GRAPH_SUCCESS;
}

IMPL_OP(DivExample).InferDataType(InferDataTypeDivExample);
