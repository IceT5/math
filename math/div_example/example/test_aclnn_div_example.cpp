...
int main()
{
    ...
    // 4. 调用aclnnDivExample第一段接口
    ret = aclnnDivExampleGetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnDivExampleGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 5. 调用aclnnDivExample第二段接口
    ret = aclnnDivExample(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnDivExample failed. ERROR: %d\n", ret); return ret);
