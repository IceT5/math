...
#include "nn_other.h"
#include "../op_graph/div_example_proto.h"
...
using std::string;
using std::vector;
#define DIV_INPUT(intputIndex, intputName, intputDtype, inputShape)                                         \
    ...
    graph.DivOp(placeholder##intputIndex);                                                                  \
    div1.set_input_##intputName(placeholder##intputIndex);                                                  \
    inputs.push_back(placeholder##intputIndex);

#define DIV_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                                          \
    ...
    graph.DivOp(placeholder##intputIndex);                                                                  \
    div1.set_input_##intputName(placeholder##intputIndex);                                                  \
    div1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                   \
    inputs.push_back(placeholder##intputIndex);

#define DIV_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    div1.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto div1 = op::DivExample("div1");
    std::vector<int64_t> xShape = {32, 4, 4, 4};
    DIV_INPUT(1, x1, inDtype, xShape);
    DIV_INPUT(2, x2, inDtype, xShape);

    DIV_OUTPUT(1, y, inDtype, xShape);

    outputs.push_back(div1);
    // 添加完毕
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    printf("%s - INFO - [XIR]: Start to div compute graph to ir session\n", GetTime().c_str());

    std::map<AscendString, AscendString> graph_options = {

    };
    uint32_t graph_id = 0;
    ret = session->DivGraph(graph_id, graph, graph_options);

    printf("%s - INFO - [XIR]: Session div ir compute graph to ir session success\n", GetTime().c_str());
