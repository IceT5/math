...
$$
  ...
  - 当scaleFromBlom为True时（axis转换为正数，numAxes为-1时表示到最后轴）：

    scaleShape为xShape[axis:axis + numAxes]

    biasShape为xShape[axis:axis + numAxes]

  - 当scaleFromBlom为False时（axis转换为正数， numAxes为-1时表示到最前轴）：
...
* `aclnnStatus aclnnScaleGetWorkspaceSize(const aclTensor *x, const aclTensor *scale, const aclTensor *bias, int64_t axis, int64_t numAxes, bool scaleFromBlom, aclTensor *y, uint64_t *workspaceSize, aclOpExecutor **executor)`
...
- **参数说明：**
  ...
  - scaleFromBlom(bool, 计算输入) host侧BOOL类型，指定要scaleFromBlom类型, True: scale from blom, 使用numAxes + axis进行scale, False: scale from input scale, 从axis开始 scale input scale长度, 忽略numAxes取值。
...
- **返回值：**
  ...
  返回161002（ACLNN_ERR_PARAM_INVALID）：1. x的数据类型不在支持的范围之内。
                                        ...
                                        9. numAxes的取值小于-1。
                                        10. scaleFromBlom为True，numAxes等于0且scale的shape不为[1]。
