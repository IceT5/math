...
#include "math/div/op_kernel/div_tiling_data.h"
#include "math/div/op_kernel/div_tiling_key.h"
...
ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            context, "Div: inputx,inputy,outputz shape dim = %zu, %zu, %zu, should be equal 4",
...
// tiling 分发入口
// 可直接替换你的 DivTilingFunc 内部实现（保留函数签名）
static ge::graphStatus DivTilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        DivTilingData* tiling = context->GetTilingData<DivTilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(DivTilingData), 0, sizeof(DivTilingData));
    ...
    DivTilingData* tiling = context->GetTilingData<DivTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(DivTilingData), 0, sizeof(DivTilingData)) != EOK,
...
static ge::graphStatus TilingParseForDiv([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(Div).Tiling(DivTilingFunc).TilingParse<DivCompileInfo>(TilingParseForDiv);
} // namespace optiling
