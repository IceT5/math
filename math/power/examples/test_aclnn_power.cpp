...
int main() {
  ...
  std::vector<int64_t> selfShape = {8,1};
  std::vector<int64_t> exponentShape = {1,1};
  std::vector<int64_t> outShape = {8,1};
  ...
  std::vector<float> selfHostData(8, 2);
  std::vector<float> exponentHostData(1, 3);
  ...
  // 调用aclnnPowTensor第一段接口
  ret = aclnnPowerGetWorkspaceSize(self, exponent, out, &workspaceSize, &executor);
  ...
  // 调用aclnnPow第二段接口
  ret = aclnnPower(workspaceAddr, workspaceSize, executor, stream);
