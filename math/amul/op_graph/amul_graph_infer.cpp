...
/*!
 * \file amul_graph_infer.cpp
 * \brief amul operater graph infer resource
 */
...
static ge::graphStatus InferDataTypeAmul(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeAmul");

    // 设置输出的dtype
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeAmul");
    return GRAPH_SUCCESS;
}

IMPL_OP(Amul).InferDataType(InferDataTypeAmul);
