...
int main()
{
    ...
    std::vector<float> selfXHostData(2048,2);
    ...
    std::vector<float> selfYHostData(2048, 3);
    ...
    // 4. 调用aclnnAmul第一段接口
    ret = aclnnAmulGetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAmulGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 5. 调用aclnnAmul第二段接口
    ret = aclnnAmul(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAmul failed. ERROR: %d\n", ret); return ret);
    ...
    std::vector<int64_t> outShape0 = {8};
