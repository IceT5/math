...
#include "math/amul/op_kernel/amul_tiling_data.h"
#include "math/amul/op_kernel/amul_tiling_key.h"
...
ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            context, "Amul: inputx,inputy,outputz shape dim = %zu, %zu, %zu, should be equal 4",
...
// tiling 分发入口
static ge::graphStatus AmulTilingFunc(gert::TilingContext* context)
{
    // 1、获取平台运行信息
    uint64_t ubSize;
    int64_t coreNum;
    ...
    // 4、设置tiling信息
    AmulTilingData* tiling = context->GetTilingData<AmulTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(AmulTilingData), 0, sizeof(AmulTilingData)) != EOK,
...
static ge::graphStatus TilingParseForAmul([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(Amul).Tiling(AmulTilingFunc).TilingParse<AmulCompileInfo>(TilingParseForAmul);
} // namespace optiling
