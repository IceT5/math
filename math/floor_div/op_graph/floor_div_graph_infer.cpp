...
/*!
 * \file floor_div_graph_infer.cpp
 * \brief floor_div operater graph infer resource
 */
...
static ge::graphStatus InferDataTypeFloorDiv(gert::InferDataTypeContext* context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeFloorDiv");

    // 设置输出的dtype FloorDiv算子逻辑是两个数floordiv，因此输出dataType与输入dataType一致
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeFloorDiv");
    return GRAPH_SUCCESS;
}

IMPL_OP(FloorDiv).InferDataType(InferDataTypeFloorDiv);
