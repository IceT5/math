...
// ----------------FLOAT32--------------------------
constexpr int32_t BUFFER_NUM_2 = 2;

template <typename T>
class FloorDiv_FLOAT {
public:
    __aicore__ inline FloorDiv_FLOAT(){};
...
template <typename T>
__aicore__ inline void FloorDiv_FLOAT<T>::Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, const FloorDivTilingData* tilingData)
{
        ASSERT(AscendC::GetBlockNum() != 0 && "block dim can not be zero!");
        uint32_t coreNum = AscendC::GetBlockIdx();
        uint32_t globalBufferIndex = tilingData->bigCoreDataNum * AscendC::GetBlockIdx();
        ...
        inputGMY.SetGlobalBuffer((__gm__ T*)y + globalBufferIndex, this->coreDataNum);
        outputGMZ.SetGlobalBuffer((__gm__ T*)z + globalBufferIndex, this->coreDataNum);
        pipe.InitBuffer(inputQueueX, BUFFER_NUM_2, this->tileDataNum * sizeof(T));
        pipe.InitBuffer(inputQueueY, BUFFER_NUM_2, this->tileDataNum * sizeof(T));
        pipe.InitBuffer(outputQueueZ, BUFFER_NUM_2, this->tileDataNum * sizeof(T));
    }

template <typename T>
__aicore__ inline void FloorDiv_FLOAT<T>::CopyIn(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.AllocTensor<T>();
    AscendC::LocalTensor<T> yLocal = inputQueueY.AllocTensor<T>();
    AscendC::DataCopy(xLocal, inputGMX[progress * this->tileDataNum], this->processDataNum);
...
template <typename T>
__aicore__ inline void FloorDiv_FLOAT<T>::CopyOut(int32_t progress)
{
    AscendC::LocalTensor<T> zLocal = outputQueueZ.DeQue<T>();
    AscendC::DataCopy(outputGMZ[progress * this->tileDataNum], zLocal, this->processDataNum);
    outputQueueZ.FreeTensor(zLocal);
...
template <typename T>
__aicore__ inline void FloorDiv_FLOAT<T>::Compute(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.DeQue<T>();
    AscendC::LocalTensor<T> yLocal = inputQueueY.DeQue<T>();
    AscendC::LocalTensor<T> zLocal = outputQueueZ.AllocTensor<T>();
...
template <typename T>
__aicore__ inline void FloorDiv_FLOAT<T>::Process()
{
    int32_t loopCount = this->tileNum;
    this->processDataNum = this->tileDataNum;
    for (int32_t i = 0; i < loopCount; i++) {
...
// ----------------INT8_UINT8-----------------------
constexpr int32_t BUFFER_NUM_1 = 2;
template <typename T>
class FloorDiv_INT8_UINT8 {
public:
    __aicore__ inline FloorDiv_INT8_UINT8(){};
...
private:
    TPipe pipe;
    TQue<QuePosition::VECIN, BUFFER_NUM_1> inputQueueX;
    TQue<QuePosition::VECIN, BUFFER_NUM_1> inputQueueY;
    TQue<QuePosition::TSCM, 1> tmp1, tmp11;
    TQue<QuePosition::VECOUT, BUFFER_NUM_1> outputQueueZ;
    GlobalTensor<T> inputGMX;
    GlobalTensor<T> inputGMY;
    GlobalTensor<T> outputGMZ;
...
template <typename T>
__aicore__ inline void FloorDiv_INT8_UINT8<T>::Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, const FloorDivTilingData* tilingData)
{
        ASSERT(AscendC::GetBlockNum() != 0 && "block dim can not be zero!");
        uint32_t coreNum = AscendC::GetBlockIdx();
        uint32_t globalBufferIndex = tilingData->bigCoreDataNum * AscendC::GetBlockIdx();
        ...
        inputGMY.SetGlobalBuffer((__gm__ T*)y + globalBufferIndex, this->coreDataNum);
        outputGMZ.SetGlobalBuffer((__gm__ T*)z + globalBufferIndex, this->coreDataNum);
        pipe.InitBuffer(inputQueueX, BUFFER_NUM_1, this->tileDataNum * sizeof(T));
        pipe.InitBuffer(inputQueueY, BUFFER_NUM_1, this->tileDataNum * sizeof(T));
        pipe.InitBuffer(outputQueueZ, BUFFER_NUM_1, this->tileDataNum * sizeof(T));
        pipe.InitBuffer(tmp1, 1, this->tileDataNum * sizeof(half));
        pipe.InitBuffer(tmp11, 1, this->tileDataNum * sizeof(half));
    }

template <typename T>
__aicore__ inline void FloorDiv_INT8_UINT8<T>::CopyIn(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.AllocTensor<T>();
    AscendC::LocalTensor<T> yLocal = inputQueueY.AllocTensor<T>();
    AscendC::DataCopy(xLocal, inputGMX[progress * this->tileDataNum], this->processDataNum);
...
template <typename T>
__aicore__ inline void FloorDiv_INT8_UINT8<T>::CopyOut(int32_t progress)
{
    AscendC::LocalTensor<T> zLocal = outputQueueZ.DeQue<T>();
    AscendC::DataCopy(outputGMZ[progress * this->tileDataNum], zLocal, this->processDataNum);
    outputQueueZ.FreeTensor(zLocal);
...
template <typename T>
__aicore__ inline void FloorDiv_INT8_UINT8<T>::Compute(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.DeQue<T>();
    AscendC::LocalTensor<T> yLocal = inputQueueY.DeQue<T>();
    AscendC::LocalTensor<T> zLocal = outputQueueZ.AllocTensor<T>();
...
template <typename T>
__aicore__ inline void FloorDiv_INT8_UINT8<T>::Process()
{
    int32_t loopCount = this->tileNum;
    this->processDataNum = this->tileDataNum;
    for (int32_t i = 0; i < loopCount; i++) {
