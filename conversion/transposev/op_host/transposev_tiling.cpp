...
#include "conversion/transposev/op_kernel/transposev_tiling_data.h"
#include "conversion/transposev/op_kernel/transposev_tiling_key.h"
...
// tiling 分发入口
// 可直接替换你的 TransposevTilingFunc 内部实现（保留函数签名）
static ge::graphStatus TransposevTilingFunc(gert::TilingContext* context)
{
    ...
    switch (inputBytes) {
        ...
        default: ubDataNumber = 13U;  break;   // 未知类型按 8 B 处理
    }
    ...
    //context->SetTilingKey(dim);
    for(int32_t i=0;i<8;i++){                                            //将输入的shape存入tiling参数
        tiling->shape[i] = static_cast<int64_t>(0);
    }
    for(int32_t i=0;i<dim;i++){                                            //将输入的shape存入tiling参数
